      ...
      if ($out_data == TRUE) {
        if ($depost > $out_data->remain_amount) {
          ...
          if ($new_balance >= $remain_amount) {
            ...
            $today = date("Y-m-d");
            $date = date("d/m/Y");
            $sms = 'Ndugu, ' . $full_name . ' ' . $date . ' Umefanya malipo ya TZS.' . number_format($new_balance) . ' ' . $comp_name . ' Mpokeaji ' . $role . ' bado unadaiwa kiasi cha TZS.' . $remain_loan;
            $massage = $sms;
            $phone = $phones;
...
// }
  ...
  public function sendsms($phone, $massage)
  {
    ...
    $api_key = '	jVNBbaRYKJYJaTkcMQ.bmzbBNT';
